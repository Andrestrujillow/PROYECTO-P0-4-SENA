...
interface ChartBarIconProps extends Omit<HTMLAttributes<HTMLDivElement>, "color" | "onDrag" | "onDragStart" | "onDragEnd" | "onAnimationStart" | "onAnimationEnd" | "onAnimationIteration"> {
  size?: number;
  duration?: number;
  isAnimated?: boolean;
  color?: string;
...
const ChartBarIcon = forwardRef<ChartBarIconHandle, ChartBarIconProps>(
  ({ onMouseEnter, onMouseLeave, className, size = 24, duration = 1, isAnimated = true, color, ...props }, ref) => {
    ...
    const chartVariants: Variants = {
      normal: { scale: 1, transition: { duration: 0.2 * duration } },
      animate: { scale: [1, 1.05, 1], transition: { duration: 0.6 * duration, ease: "easeInOut" } },
    };
